...
experimental_metadata = read.delim("construct/metadata_ubi.txt", sep=",", header=TRUE, stringsAsFactors=FALSE)
...
oksm_long <- oksm_long %>%
  mutate(construct = c(rep("myc", 11), rep("oks", 11)),
...
oksm_long %>%
  ...
  scale_x_discrete(limits = c("AT_TdTom_1","AT_TdTom_2","AT_TdTom_3","AT_OKSM_1","AT_OKSM_2","AT_OKSM_3","AT_Sen_2","AT_Sen_3","AT_Sen_OKSM_1","AT_Sen_OKSM_2","AT_Sen_OKSM_3"))
```
...
oksm_long %>%
  ...
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank()) +
  scale_fill_manual(values = c("#BF3100", "#E9B44C", "#1B998B", "#5D576B"))+
  geom_point(data=oksm_long, aes(x=condition, y=value), size=2, position="dodge") + facet_wrap(~construct)
```
