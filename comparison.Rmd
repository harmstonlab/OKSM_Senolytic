...
ubi_pam_clust1_genes <- ubi_pam_clust_df[ubi_pam_clust_df$Cluster == 2, ] %>%
  mutate(Cluster = 1)
ubi_pam_clust2_genes <- ubi_pam_clust_df[ubi_pam_clust_df$Cluster == 3, ] %>%
  mutate(Cluster = 2) 
ubi_pam_clust3_genes <- ubi_pam_clust_df[ubi_pam_clust_df$Cluster == 4, ] %>%
  mutate(Cluster = 3)
ubi_pam_clust4_genes <- ubi_pam_clust_df[ubi_pam_clust_df$Cluster == 7, ] %>%
  mutate(Cluster = 4)
ubi_pam_clust5_genes <- ubi_pam_clust_df[ubi_pam_clust_df$Cluster == 5, ]
ubi_pam_clust6_genes <- ubi_pam_clust_df[ubi_pam_clust_df$Cluster == 6, ]
ubi_pam_clust7_genes <- ubi_pam_clust_df[ubi_pam_clust_df$Cluster == 1, ] %>%
  mutate(Cluster = 7)
ubi_pam_clust_df = rbind(ubi_pam_clust1_genes, ubi_pam_clust2_genes, ubi_pam_clust3_genes, 
                         ubi_pam_clust4_genes, ubi_pam_clust5_genes, ubi_pam_clust6_genes, ubi_pam_clust7_genes)
```
...
```{r}
overlapping_genes <- ubi_pam_clust_df[rownames(ubi_pam_clust_df) %in% rownames(isc_pam_clust_df),]
overlapping_genes %>%
  dplyr::select(gene_name, Cluster) %>%
...
`r sum(tab$n)` genes in Cluster 1 of the ubiquitous dataset is accounted for in the ISC dataset. This implies that `r length(denom)-sum(tab$n)` genes in C1 of the ubiquitous dataset is not expressed or significantly differentially expressed in the ISC dataset. 
...
`r sum(tab$n)` genes in Cluster 3 of the ubiquitous dataset is accounted for in the ISC dataset. This implies that `r length(denom)-sum(tab$n)` genes in C3 of the ubiquitous dataset is not expressed or significantly differentially expressed in the ISC dataset. 
...
`r sum(tab$n)` genes in Cluster 4 of the ubiquitous dataset is accounted for in the ISC dataset. This implies that `r length(rownames(isc_c6))-sum(tab$n)` genes in C4 of the ubiquitous dataset is not expressed or significantly differentially expressed in the ISC dataset. 
