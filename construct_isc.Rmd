...
experimental_metadata = read.delim("construct/metadata_isc.txt", sep=",", header=TRUE, stringsAsFactors=FALSE)
...
data = matrix(0, ncol=length(experimental_metadata$sample_id), nrow=17716)
...
oksm_long <- oksm_long %>%
  mutate(construct = c(rep("myc", 12), rep("oks", 12)),
         condition = factor(
           ifelse(str_sub(oksm_long$name,1,-5) %in% c("C1", "C2", "C3"), "Control",
                  ifelse(str_sub(oksm_long$name,1,-5) %in% c("O1", "O2", "O3"), "OKSM",
                         ifelse(str_sub(oksm_long$name,1,-5) %in% c("S1", "S2", "S3"), "Sen", "Sen_OKSM"))), 
...
oksm_long %>%
  ...
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank()) +
  scale_fill_manual(values = c("#BF3100", "#E9B44C", "#1B998B", "#5D576B"))+
  geom_point(data=oksm_long, aes(x=condition, y=value), size=2) + facet_wrap(~construct) + scale_y_continuous(limits=c(0, 1000))
